...
<h1 align="center" style="font-weight: bold; ">Kolkata Knight Riders</h1>
<a href="addKolkata" class="btn btn-info" style="font-weight: bold;">Add New Player</a>
<table class="table mt-5 table-info ">
  ...
  <tbody>
   <c:forEach var="plr" items="${player}">
    <tr>
      <td><a href="kolkata/${plr.name}"/><c:out value="${plr.name}"/></td>
      <td><c:out value="${plr.role}"/></td>
      <td><c:out value="${plr.price}"/></td>
      <td><c:out value="${plr.nationality}"/></td>
      <td><a href="editKolkata/<c:out value="${plr.name}"/>" class="btn btn-warning">Edit</a>
      <a href="deleteKolkata/<c:out value="${plr.name}"/>" class="btn btn-danger">Delete</a>
      </td>
    </tr>
    </c:forEach>
  </tbody>
