...
<h1 align="center" style="font-weight: bold; ">Punjab Kings</h1>
<a href="addPunjab" class="btn btn-info" style="font-weight: bold;">Add New Player</a>
<table class="table mt-5 table-info ">
  ...
  <tbody>
   <c:forEach var="plr" items="${player}">
    <tr>
      <td><a href="punjab/${plr.name}"/><c:out value="${plr.name}"/></td>
      <td><c:out value="${plr.role}"/></td>
      <td><c:out value="${plr.price}"/></td>
      <td><c:out value="${plr.nationality}"/></td>
      <td><a href="editPunjab/<c:out value="${plr.name}"/>" class="btn btn-warning">Edit</a>
      <a href="deletePunjab/<c:out value="${plr.name}"/>" class="btn btn-danger">Delete</a>
      </td>
    </tr>
    </c:forEach>
  </tbody>
